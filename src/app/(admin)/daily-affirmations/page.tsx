...
export default function DailyAffirmationsPage() {
  ...
  const fetchAffirmations = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/daily-affirmations?${params}`);
      const data = await res.json();
      if (data.success) {
        setItems(data.affirmations);
      }
    } catch (e) {
      console.error("Failed to load daily affirmations", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? `/api/admin/daily-affirmations/${editingId}`
        : "/api/admin/daily-affirmations";
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/daily-affirmations/${confirmDialog.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
      ...
      {(showForm || editingId) && (
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 relative">
          ...
          <form onSubmit={handleSubmit} className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              <DatePicker
                key={`daily-scheduled-${editingId || 'new'}`}
                id="daily-affirmation-scheduled-for-picker"
